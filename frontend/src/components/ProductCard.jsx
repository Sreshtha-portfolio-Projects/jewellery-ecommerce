    ...
    <div className="group relative bg-white rounded-lg overflow-hidden shadow-sm hover:shadow-lg transition-all duration-300">
      <Link to={`/product/${product.id}`} className="block">
        ...
        <div className="p-3 sm:p-4">
          <h3 className="font-serif text-base sm:text-lg font-semibold text-gray-900 mb-2 group-hover:text-rose-700 transition-colors line-clamp-2">
            {product.name}
          </h3>
          <div className="flex items-center justify-between">
            <span className="text-lg sm:text-xl font-semibold text-gray-900">${parseFloat(product.price).toFixed(2)}</span>
      ...
      <div className="px-3 sm:px-4 pb-3 sm:pb-4">
        <button 
          ...
          className="w-full py-2 text-sm sm:text-base bg-rose-600 text-white rounded hover:bg-rose-700 active:bg-rose-800 transition-colors opacity-0 group-hover:opacity-100 md:opacity-100 md:group-hover:opacity-100"
        >
          Add to Cart
        </button>
      </div>
